...
export function OverviewActionStatusChart({
  slices,
  total,
  hk,
}: {
        ...
        <div className="relative shrink-0">
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Actions 状态分布环形图">
            ...
            {arcs.map(({ slice, start, end, sweep }) => {
              ...
              if (sweep >= 359.9) {
                return <circle key={slice.key} cx={cx} cy={cy} r={r} fill="none" stroke={slice.color} strokeWidth={stroke} />;
              }
              return (
                <path
                  key={slice.key}
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-muted-foreground text-xs">总 Actions</span>
          </div>
        </div>
          ...
          {slices.map((slice) => (
            <li key={slice.key} className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground tabular-nums">
                {slice.count} <span className="text-foreground">({slice.percent}%)</span>
              </span>
            </li>
          ))}
        </ul>
