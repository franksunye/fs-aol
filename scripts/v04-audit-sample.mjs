#!/usr/bin/env node
/**
 * v0.4 acceptance: sample follow-up rows and verify trace/outcome/action linkage.
 */
...
async function main() {
  ...
  for (const row of sample.rows) {
    ...
    const decision = row.decision ?? row[3];
    const bucket = row.inbox_bucket ?? row[2] ?? "active";
    const hasTrace = traces > 0;
    const needsAction =
      decision === "approved" || decision === "modified";
    const actionOk = !needsAction || actions > 0 || bucket === "closed";
    const pass = hasTrace && actionOk;
    if (pass) ok++;
    else fail++;
    console.log(
